...
class TextmaskControl extends Component {
	...
	handleSubmit = () => {
		...
		if (nameExists != "yes") {
			...
			if (this.state.inputMask !== undefined) {
				var inputMask = this.state.inputMask;
			}
			...
			if (this.state.patternValMsg !== undefined) {
				var patternValMsg = this.state.patternValMsg;
			}
	...
	render() {
		return (
			<Fragment>
				<div>
					<div>
						<h5><b>Textmask Configuration</b></h5>
					</div>
					<div>
						<Input
							s={12}
...
export default TextmaskControl;